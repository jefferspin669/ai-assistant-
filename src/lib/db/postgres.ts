...
export async function persistAtlasDatabase(data: AtlasDatabase): Promise<void> {
  ...
  await upsertRows(
    ...
    data.memories.map((m) => ({
      ...
      userId: m.userId,
      kind: m.kind,
      memoryType: m.memoryType || "operational",
      title: m.title,
      content: m.content,
      source: m.source || "system",
      authorLabel: m.authorLabel || "Atlas",
      confidence: m.confidence ?? 80,
      accessLevel: m.accessLevel || "all_staff",
  ...
  await upsertRows(
    schema.memoryOutcomes,
    (data.memory_outcomes || []).map((o) => ({
      id: o.id,
...
export async function loadAtlasDatabaseFromPostgres(): Promise<AtlasDatabase | null> {
  ...
  const [
    ...
    selectAll<typeof schema.projects.$inferSelect>(schema.projects),
    ...
    selectAll<typeof schema.memoryOutcomes.$inferSelect>(schema.memoryOutcomes),
  ...
  return {
    ...
    projects: projects.map((p) => ({
      ...
      name: p.name,
      description: p.description,
      status: p.status as AtlasDatabase["projects"][number]["status"],
      createdBy: p.createdBy,
      createdAt: p.createdAt,
      updatedAt: p.updatedAt,
    })),
    ...
    memories: memories.map((m) => ({
      ...
      memoryType: (m.memoryType || "operational") as AtlasDatabase["memories"][number]["memoryType"],
      title: m.title,
      content: m.content,
      source: m.source || "system",
      authorLabel: m.authorLabel || "Atlas",
      confidence: m.confidence ?? 80,
      accessLevel: (m.accessLevel || "all_staff") as AtlasDatabase["memories"][number]["accessLevel"],
      entityType: m.entityType ?? null,
      entityId: m.entityId ?? null,
      approved: m.approved,
      createdAt: m.createdAt,
    ...
    memory_outcomes: memoryOutcomes.map((o) => ({
      id: o.id,
      organizationId: o.organizationId,
      memoryId: o.memoryId,
      recommendation: o.recommendation,
      status: o.status as AtlasDatabase["memory_outcomes"][number]["status"],
      original: o.original,
      edited: o.edited,
      actorUserId: o.actorUserId,
      createdAt: o.createdAt,
