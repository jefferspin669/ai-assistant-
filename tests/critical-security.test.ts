...
import { validateTwilioSignature } from "../src/lib/integrations/twilio-security";
...
describe("critical security fixes", () => {
  ...
  it("hashes server passwords with scrypt and upgrades legacy hashes on login", () => {
    ...
    // Leave plaintext legacy hash; authenticate should verify then upgrade to scrypt.
    saveDatabase({
      ...db,
  ...
  it("ignores client approved flags and requires a consumed server confirmation for SMS", async () => {
    ...
    saveDatabase({
      ...
      approvals: database().approvals.map((a) =>
        a.id === staged.approvalId
          ? { ...a, status: "approved" as const, resolved_at: new Date().toISOString() }
          : a,
      ),
    });
  ...
  it("validates Twilio request signatures", () => {
    ...
    let data = url;
    for (const key of sortedKeys) data += key + params[key as keyof typeof params];
